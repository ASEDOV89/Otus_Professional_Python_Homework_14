...
def insert_appsinstalled(memc_addr, appsinstalled, dry_run=False):
    logging.info(
        f"Attempting to insert data for {appsinstalled.dev_id} at {memc_addr}"
    )
    ...
    try:
        if dry_run:
            logging.debug(
                f"{memc_addr} - {key} -> {str(ua).replace('\n', ' ')}"
            )
        else:
...
def process_file(fn, device_memc, dry_run):
    ...
    with gzip.open(fn, 'rt', encoding='utf-8') as fd:
        for line in fd:
            ...
            if not memc_addr:
                errors += 1
                logging.error(
                    f"Unknown device type: {appsinstalled.dev_type}"
                )
                continue
    ...
    if err_rate < NORMAL_ERR_RATE:
        ...
        logging.error(
            f"High error rate ({err_rate} > {NORMAL_ERR_RATE}). Failed load"
        )
    dot_rename(fn)
...
def main(options):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {executor.submit(
            process_file, fn, device_memc, options.dry
        ): fn for fn in files}
...
if __name__ == '__main__':
    ...
    op.add_option(
        "--pattern", action="store", default="*.tsv.gz"
    )
    ...
    logging.basicConfig(
        filename=opts.log, level=logging.INFO if not opts.dry else logging.DEBUG,
